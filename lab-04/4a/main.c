...
char *process(const char *str) {
    char *s = strdup(str); // копируем строчку которую ввел пользователь. Зовут его s.
    if (s == NULL){
        return 0;
    }
    int s_len = strlen(s); // Длина строки которую ввел пользователь.
    ...
    int len = 0;
    char *word = strtok(s, DELIM); // strtok - выделяет слова. Delim-пробелы.
    ...
    while (word != NULL) { // strtok может быть равен NULL, если больше токенов не найдено.
        ...
        for (int i = 0; word[i] != '\0'; i++){ // проверка на гласные буквы
                ...
                if (count % 2 != 0 || count == 0) {
                        ...
                        int w_len = strlen(word); // если не было слова, то мы берем след. слово
                        memcpy(res + len, word, w_len * sizeof(char)); // копируем след. слово
                        len += w_len;
        }
